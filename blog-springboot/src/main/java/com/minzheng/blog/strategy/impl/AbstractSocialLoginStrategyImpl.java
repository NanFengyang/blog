...
public abstract class AbstractSocialLoginStrategyImpl implements SocialLoginStrategy {
    ...
    public UserInfoDTO login(String data) {
        ...
        System.out.println("login>>>socialToken>>>"+socialToken);
        // 获取用户ip信息
    ...
    private UserDetailDTO saveUserDetail(SocialTokenDTO socialToken, String ipAddress, String ipSource) {
        ...
        // 保存账号信息
        //判断用户名是否存在，存在先删除该用户，在插入
        UserAuth selectOne = userAuthDao.selectOne(new LambdaQueryWrapper<UserAuth>()
                .eq(UserAuth::getUsername, socialToken.getOpenId()));
        if (Objects.nonNull(selectOne)){
            userAuthDao.delete(new LambdaQueryWrapper<UserAuth>()
                    .eq(UserAuth::getUsername, socialToken.getOpenId()));
        }
